...
{
	cout << endl;
	cout << prog_name << " Version " << PROJECT_VERSION << endl
		 << ". Usage:"
		 << endl;
...
po::options_description LicenseGenerator::configureProgramOptions()
{
	...
	licenseGeneration.add_options()("private_key,p", po::value<string>(),
		...
		("end_version,n", po::value<unsigned int>()->default_value(0
		 ...
		 ("extra_data,u", po::value<string>(),"user information about license."); //
	...
	po::variables_map vm)
{
	...
	if (vm.count("expire_date"))
	{
		...
		{
			cerr << endl
				 << "End date not recognized: " << dt_end
	...
	if (vm.count("begin_date"))
	{
		...
		{
			cerr << endl
				 << "Begin date not recognized: " << begin_date_str
				 << " Please enter a valid date in format YYYYMMDD" << endl;
			//print_usage(vm);
			exit(2);
		}
	...
	{
		cerr << endl
			 << "Parameter [product] not found. " << endl;
		exit(2);
	}
...
int LicenseGenerator::generateLicense(int argc, const char **argv)
{
	...
	allOptions.add(visibleOptions).add_options()("product", po::value<vector<string>>(), "product names");
